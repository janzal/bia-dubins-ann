...
namespace imr {
   namespace gui {
      namespace manager {
         ...
         /// ----------------------------------------------------------------------------
         class CObject : public imr::gui::render::SRendererObject {
            public:
               typedef imr::gui::manager::TID TID;
               typedef imr::gui::manager::TagList TagList;
            protected:
               static TID maxID;
               TID id;
               TDepth depth;
               TagList tags;
            public:
               CObject(ObjectType type, std::string tags, const CShape& shape, TDepth depth = TDepth(0));
               virtual ~CObject() {}
               inline const TID & getID(void) const {
                  return id;
               }
               inline const TDepth & getDepth(void) const {
                  return depth;
               }

               TagList::iterator tags_begin(void) {
                  return tags.begin();
               }

               TagList::iterator tags_end(void) {
                  return tags.end();
               }
         ...
         class CIDTree {
            public:
               ...
               ObjectMap::iterator find(const TID & id) {
                  return objectMap.find(id);
               }

               void erase(const TID & id) {
                  objectMap.erase(id);
               }

               ObjectMap::iterator objects_begin(void) {
                  return objectMap.begin();
               }

               ObjectMap::iterator objects_end(void) {
         ...
         class CDepthTree {
            private:
               struct DepthLess { 
                  bool operator()(const TDepth& d1, const TDepth& d2) const { return d1 > d2; } 
               };
            public:
               void add(CObject * object);
         ...
         class CManager {
            public:
               ...
               typedef imr::gui::manager::ObjectList ObjectList;
