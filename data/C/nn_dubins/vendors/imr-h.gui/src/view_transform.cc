...
static imr::CLoggerPtr loclog = 0;

/// ----------------------------------------------------------------------------
static void init(void) {
   if (!loclog) {
      loclog = imr::getLogger("gui");
   }
}
...
CViewTransform::CViewTransform() : 
   viewX(640), viewY(480), viewBorderX(10), viewBorderY(10),
   realMinX(0.), realMinY(0.), realMaxX(0.), realMaxY(0.),
   kX(viewX / realMaxX), kY(viewY / realMaxY), 
   ratioX(fabs(kX)), ratioY(fabs(kY)),
   ar(-1.0)
{
   init();
   compute();
...
/// ----------------------------------------------------------------------------
void CViewTransform::setAspectRation(double arI) {
   ar = arI;
}
...
/// ----------------------------------------------------------------------------
imr::CBBox& CViewTransform::getBBox(imr::CBBox& bbox) const {
   ...
   if (ar > 0.0) { // width / height
      ...
      if (a > ar) { // h_coords must be increased
         ...
         if (optimalH != viewY) {
            realMinY -= dy;
            realMaxY += dy;
            DEBUG_M(loclog, "For aspect ratio " << ar << " optimal canvas size is " << viewX << "x" << optimalH);
            perfect = false;
         }
      } else if (a < ar) {
         const double k = ar / a;
         ...
         if (optimalW != viewX) {
            realMinX -= dx;
            realMaxX += dx;
            DEBUG_M(loclog, "For aspect ratio " << ar << " optimal canvas size is " << optimalW << "x" << viewY);
            perfect = false;
         }
      } else { //current aspection ration is same as required
      }
      if (perfect) {
         DEBUG_M(loclog, "Canvas size perfectly match current real coords with required aspect ratio");
      }
   }
   kX = (viewX - 2. * viewBorderX ) / (realMaxX - realMinX); // -1 for start from 0 to width-1 
   kY = (viewY - 2. * viewBorderY ) / (realMaxY - realMinY); // -1 for start from 0 to height-1
