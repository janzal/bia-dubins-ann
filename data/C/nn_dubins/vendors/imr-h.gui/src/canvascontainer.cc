...
/// ----------------------------------------------------------------------------
SCanvasObject::SCanvasObject(ObjectType type, const std::string& tags, const CShape& shape) : CObject(type, tags, shape), name(tags) {
}
...
void SCanvasObject::draw(cairo_t* cr) {
   ...
   if (size == 1) {
      ...
      imr::gui::cairo::markers::CCirclePoint::draw(cr, coords.begin()->x, coords.begin()->y, shape);
   } else {
      if (size > 0) {
         switch(type) {
            case imr::gui::render::POLYGON:
               {
                  cairo_new_path(cr);
                  if (shape.getOutline()) {
            ...
            case imr::gui::render::ARC:
               ...
               break;
	    case imr::gui::render::SEGMENT_SET:
	       {
		  cairo_save(cr);
		  cairo_set_line_width(cr, shape.getPenWidth());
		  setColor(shape.getPenColor(), cr);
...
bool SCanvasObject::in_stroke(cairo_t* cr, double x, double y) {
   ...
   if (size == 1) {
      //TODO introduce zoom
      setColor(shape.getPenColor(), cr);
      ret |= imr::gui::cairo::markers::CCirclePoint::in_stroke(cr, coords.begin()->x, coords.begin()->y, x, y, shape);
   } else {
      if (size > 0) {
         switch(type) {
            case imr::gui::render::POLYGON:
...
/// ----------------------------------------------------------------------------
void CCanvasDrawer::setCairo(cairo_t* iCr) {
   cr = iCr;
}

/// ----------------------------------------------------------------------------
void CCanvasDrawer::execute(imr::gui::manager::CObject* obj) {
...
/// ----------------------------------------------------------------------------
class CCanvasObjectNameFinder : public imr::gui::manager::CObjectExecutor {
   cairo_t* cr;
   double x;
   double y;
   std::list<std::string>& lst;
   public:
   CCanvasObjectNameFinder (cairo_t* cr, double x, double y, std::list<std::string>& lst) : cr(cr), x(x), y(y), lst(lst) {}
...
/// ----------------------------------------------------------------------------
class CCanvasObjectFinder : public imr::gui::manager::CObjectExecutor {
   cairo_t* cr;
   double x;
   double y;
   imr::gui::manager::CManager::ObjectList& lst;
   public:
   CCanvasObjectFinder (cairo_t* cr, double x, double y, imr::gui::manager::CManager::ObjectList& lst) : cr(cr), x(x), y(y), lst(lst) {}
   void execute(imr::gui::manager::CObject* obj) {
      SCanvasObject* o = dynamic_cast<SCanvasObject*>(obj);
      if (o && o->in_stroke(cr, x, y)) {
         lst.push_back(o);
...
/// ----------------------------------------------------------------------------
void CCanvasContainer::drawBackground(cairo_t* cr, int width, int height, double zoom) {
   cairo_set_source_rgba (cr, 1.0, 1.0, 1.0, 1.0);
   cairo_rectangle (cr, 0, 0, width/zoom, height/zoom);
   cairo_fill(cr);
   cairo_set_source_rgba (cr, backgroundColor.red, backgroundColor.green, backgroundColor.blue, backgroundColor.alpha);
   cairo_rectangle (cr, 0, 0, width/zoom, height/zoom);
   cairo_fill(cr);
}
...
/// ----------------------------------------------------------------------------
void CCanvasContainer::getObjectsNames(double x, double y, std::list<std::string>& lst) {
   ScopedLock lk(mtx);
   if (cr) {
...
/// ----------------------------------------------------------------------------
void CCanvasContainer::getObjects(double x, double y, Manager::ObjectList & objects) {
   ScopedLock lk(mtx);
   if (cr) {
