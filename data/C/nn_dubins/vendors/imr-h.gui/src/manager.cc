...
using namespace imr::gui::manager;
...
/// Class CObject
/// ----------------------------------------------------------------------------
TID imr::gui::manager::CObject::maxID = 0;

/// ----------------------------------------------------------------------------
CObject::CObject(ObjectType type, std::string tags, const CShape& shape, TDepth depth) : imr::gui::render::SRendererObject(type, shape), id(maxID++), depth(depth) {
...
/// Class CIDTree
/// ----------------------------------------------------------------------------
void CIDTree::add(CObject * object) {
...
void CDepthTree::execute(CObjectExecutor * executor) {
   for (DepthMap::iterator it = depthMap.begin(); it != depthMap.end(); it++) {
      ...
      for (ObjectMap::iterator obj = tIDTree->objects_begin(); obj != tIDTree->objects_end(); obj++) {
...
void CManager::add(CObject * object) {
   ...
   for (TagList::iterator it = object->tags_begin(); it != object->tags_end(); it++) {
...
void CManager::remove(Object::TID id) {
   ...
   CObject * obj = 0;
   while(it != tree.end()) {
      if (it->second->find(id) != it->second->objects_end()) { //id is in the tree
...
void CManager::find(std::string tag, ObjectList & list) {
   ...
   for (Tokenizer::iterator tag = tok.begin(); tag != tok.end(); tag++) {
      ...
      if (objects != tree.end()) {
         for (ObjectMap::iterator it = objects->second->objects_begin(); it != objects->second->objects_end(); it++) {
   ...
   if (tag == "all") {
      ...
      if (it != tree.end()) { //tag already presented in tag tree
         ...
         while((obj = tidree->first()) != 0) {
            for (TagList::iterator t = obj->tags_begin(); t != obj->tags_end(); t++) {
               if (*t != tag) {
