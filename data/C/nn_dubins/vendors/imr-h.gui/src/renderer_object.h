...
namespace imr { namespace gui { namespace render {
   ...
   class CCoordsTransform {
	 ...
	 virtual imr::CBBox& getBBox(imr::CBBox& bbox) const { return bbox; }
   };
   ...
   class CView2Real : public CCoordsTransform {
      public:
	 CView2Real(const CViewTransform & transform) : CCoordsTransform(), transform(transform) {}
	 virtual ~CView2Real() {}
	 ...
	 virtual std::string getTransformParam(void) const { return transform.getParam(); }
	 virtual imr::CBBox& getBBox(imr::CBBox& bbox) const { return transform.getBBox(bbox); }
      private:
   ...
   class CReal2View : public CCoordsTransform {
      public:
	 CReal2View(const CViewTransform & transform) : CCoordsTransform(), transform(transform) {}
	 virtual ~CReal2View() {}
   ...
   struct SRendererObject {
	 ...
	 Coord(double x, double y) : x(x), y(y) {}
      ...
      typedef imr::gui::CShape CShape;
      typedef imr::gui::render::TObjectType ObjectType;
