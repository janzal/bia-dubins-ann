...
#include "canvas_object_saver.h"

#include "renderercairo.h"

using namespace imr::gui::render;
...
/// ----------------------------------------------------------------------------
CRendererCairo::CRendererCairo(CCanvasContainer* iCanvas, CCairoWin* iWin) : canvas(iCanvas), win(iWin) {
...
/// ----------------------------------------------------------------------------
CRendererCairo::CRendererCairo(CCanvasContainer* iCanvas, CCairoWin* iWin, CConfig& cfg) : canvas(iCanvas), win(iWin) {
   transform.setAspectRation(cfg.get<double>("gui-aspect-ratio"));
   transform.setView(win->getWidth(), win->getHeigth());
   transform.setViewBorder(cfg.get<int>("gui-border-x"), cfg.get<int>("gui-border-y"));
   setArea(0, win->getWidth(), 0, win->getHeigth());
   antialiasing = cfg.get<bool>("gui-antialiasing");
   exportWidth = win->getWidth();
   exportHeight = win->getHeigth();
//   setExportDimension(cfg.get<int>("gui-export-width"), cfg.get<int>("gui-export-height"));
   _obj = 0;
   state = NUMBER;
}
...
/// ----------------------------------------------------------------------------
void CRendererCairo::redraw(void) {
   win->redraw();
}

/// ----------------------------------------------------------------------------
void CRendererCairo::drawPoint(double iX, double iY, const std::string& tags, const CShape& shape) {
   double x, y;
...
void CRendererCairo::clearObjects(const std::string& tags) {
   ...
   win->invalidate(); //TODO a local invalidation should be helpfull to increase performance
}
...
/// ----------------------------------------------------------------------------
//void CRendererCairo::initialization(int argc, char ** argv, CWaitObject* iRendererInit) {
/*  if (!initialized) {
    initialized = true;
    RendererTK::tk_initialization(argc, argv, iRendererInit); //TK gui initialization
    } else {
    error(logger, "CRendererTK initialization was already called");
    }*/
//}

/// ----------------------------------------------------------------------------
bool CRendererCairo::isInitialized(void) {
   return initialized;
}

/// ----------------------------------------------------------------------------
bool CRendererCairo::open(TObjectType objectType, const std::string& tags, const CShape& shape) {
   bool ret = false;
   if (state == NUMBER) {
      count = 0;
      switch(objectType) {
...
/// ----------------------------------------------------------------------------
bool CRendererCairo::save(const std::string& filename) { // check creation 
   bool ret = false;
   bool png = false;
   cairo_surface_t *surface = 0;
   if (filename.find(".pdf") != std::string::npos) {
      surface = cairo_pdf_surface_create(filename.c_str(), exportWidth, exportHeight);
   } else if (filename.find(".png") != std::string::npos) {
      surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, exportWidth, exportHeight);
      png = true;
   } else if (filename.find(".svg") != std::string::npos) {
      surface = cairo_svg_surface_create(filename.c_str(), exportWidth, exportHeight);
   } else if (filename.find(".eps") != std::string::npos) {
      surface = cairo_ps_surface_create(filename.c_str(), exportWidth, exportHeight);
      ...
      ERROR("File suffix is not support, can not save canvas into '" + filename + "'");
   }
   if (surface) {
      ...
      cairo_destroy(cr);
      if (png) {
         ret = cairo_surface_write_to_png(surface, filename.c_str()) == CAIRO_STATUS_SUCCESS;
      }
...
unsigned char* CRendererCairo::getGrayscale(int& width, int& height) {
   unsigned char* ret = 0;
   cairo_surface_t* surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, exportWidth, exportHeight);
   cairo_t* cr = cairo_create(surface);
   cairo_set_antialias(cr, CAIRO_ANTIALIAS_NONE);
   cairo_set_tolerance(cr, 0.1);
   canvas->drawBackground(cr, exportWidth, exportHeight, 1.0);
   canvas->draw(cr);
   cairo_show_page(cr);
   assert_argument(cairo_status(cr) == CAIRO_STATUS_SUCCESS, "CRendererCairo::getGrayscale draw fail");
   cairo_destroy(cr);
...
/// ----------------------------------------------------------------------------
bool CRendererCairo::shutdownRequest(void) {
   return win->shutdownRequest();
}

/// ----------------------------------------------------------------------------
void CRendererCairo::getObjectsNames(double x, double y, std::list<std::string>& lst) {
   canvas->getObjectsNames(x, y, lst);
}

/// ----------------------------------------------------------------------------
void CRendererCairo::getObjects(double x, double y, Manager::ObjectList& objects) {
...
/// ----------------------------------------------------------------------------
const imr::gui::CViewTransform& CRendererCairo::getTransform(void) {
   return transform;
}
