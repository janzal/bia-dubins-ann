...
/// Class CCanvasBase
/// ----------------------------------------------------------------------------
CCanvasBase::CCanvasBase(CGui& gui) : gui(gui), renderer(gui.getRenderer()), shape("black", "black", 1, 1) {
   dNum = 0;
   param = canvas::POINT;
...
/// ----------------------------------------------------------------------------
CRenderer* CCanvasBase::getRenderer(void) {
   return renderer;
}

/// ----------------------------------------------------------------------------
void CCanvasBase::redraw(void) {
   renderer->redraw();
};
...
/// ----------------------------------------------------------------------------
void CCanvasBase::drawLine(const double x1, const double y1, const double x2, const double y2) {
...
/// ----------------------------------------------------------------------------
CShape CCanvasBase::getShape(void) {
   return shape;
}

/// ----------------------------------------------------------------------------
std::string CCanvasBase::getTags(void) {
   return tags;
}
      ...
      case canvas::LINESTRING:
         ...
         switch(dNum) {
            ...
            case 4: //line is fully defined
               renderer->addPoint(x, d);
               renderer->close(); 
               renderer->open(render::LINE, tags, shape); //start new line immediately
               renderer->addPoint(x, d);
               dNum = 2;
               break;
         }
...
void CCanvasBase::drawArrow() {
   ...
   if (drawArrow) { 
      ...
      *this 
         << (shape.getFill() ? canvas::POLYGON : canvas::LINESTRING) 
...
/// ----------------------------------------------------------------------------
imr::gui::CCanvasBase* operator<<(imr::gui::CCanvasBase* canvas, const std::string& iTags) {
   *canvas << iTags;
   return canvas;
}

/// ----------------------------------------------------------------------------
imr::gui::CCanvasBase* operator<<(imr::gui::CCanvasBase* canvas, const imr::gui::CShape& shape) {
   *canvas << shape;
   return canvas;
}
