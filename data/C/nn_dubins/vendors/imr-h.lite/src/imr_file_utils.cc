...
namespace imr {
   ...
   std::string& readFile(const std::string &filename, std::string& content) throw(imr::io_error) {
      ...
      assert_io(ifs, "Can not open file '" + filename + "'");
      oss << ifs.rdbuf();
   ...
   std::string firstFileToken(const std::string& filename) throw(imr::io_error) {
      ...
      assert_io(ifs, "Can not open file '" + filename + "'");
      ifs >> ret;
      assert_io(ifs, "Error reading file '" + filename + "'");
      ifs.close();
      return ret;
   }
   ...
   std::string getBasename(const std::string& filename) {
      ...
      return pos == std::string::npos ? filename : filename.substr(pos + 1, filename.size());
   }
   ...
   bool createDirectory(const std::string& path) {
      ...
      if (!ret && !checkPath(path) && path.size() > 0) { //given path does not exits
         ...
         for (i = 0; i < len; i++) {
            ...
            if (path[i] == '/') {
               path[i+1] = '\0';
               if (!isDirectory(path) && (mkdir(path, S_IRWXU | S_IRWXG | S_IRWXO) != 0)) {
                  break;
               }
            }
         } //end for loop
         if (i == len) {
            path[i] = '\0';
            ret =  isDirectory(path) || (mkdir(path, S_IRWXU | S_IRWXG | S_IRWXO) == 0);
         }
         delete[] path;
      }
      return ret;
   ...
   bool isBZ2(const std::string& path) {
      ...
      char h[3];
      return (in >> h[0] >> h[1] >> h[2] and h[0] == 'B' and h[1] == 'Z' and h[2] == 'h');
   }
